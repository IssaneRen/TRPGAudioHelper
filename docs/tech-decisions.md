...
用户希望在第二个 TAB「工具箱」中新增一个“世界 wiki”功能，用于检索人物、地点、事件、模组信息；同时要求词条内容继续走静态文件链路，后续仅修改 wiki 内容时不需要重新 build 网站。

### 决策

- **路由位置**：作为工具箱子路由新增 `/tools/world-wiki` 与 `/tools/world-wiki/:entryId`
- **静态数据**：索引放在 `public/wiki/index.json`，正文放在 `public/wiki/entries/*.md`
...
- **隐藏档案机制**：支持 `[secret players="..."]...[/secret]` 语法块，根据当前 PL 名称决定是否解锁；未解锁时渲染黑色遮罩并点击 toast 提示
- **PL 状态**：沿用博客已使用的 `blog-pl-name` localStorage 键，保证跨页面一致

### 后续关注

- 若未来需要行内级别的剧透遮罩，可在现有块级语法之上追加更细粒度的 Markdown 解析规则
- 若 wiki 词条数量继续增长，可考虑补充自动索引脚本，减少手写 `index.json` 的维护成本
