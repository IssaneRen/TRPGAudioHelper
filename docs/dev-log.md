...
| 20:15 | 团队负责人 | master | **博客内容 Agent 工作流**: 多轮外网调研（混合人机流水线、战报结构、模组写作、Content CI）+ 项目约束对照；新增 4 子 Skill、6 内容 Agent、总编排 `blog-content-pipeline`、调研报告 | .claude/skills/*.md, .claude/agents/content-*.md, docs/reports/2026-05-28-blog-content-workflow-research.md, CLAUDE.md, team-leader-work.md |
| 21:40 | 团队负责人 | master | **Skills 深化 + 战役术语 + 苦痛三圣母试点**: 新增 `trpg-content-terminology`/`write-module-intro`；模组详情改「模组导读」+ `:::spoiler` 黑框；战役拆 3 条 module（暗影之屋/叹息沙漠/泪之河） | .claude/skills/*.md, src/features/modules/ModuleDescription.tsx, public/wiki/entities/modules.json, WorldWikiModuleDetailTab.tsx |
| 2026-06-01 | Cursor Agent | master | **录入莱纳·托尔拓车卡数据**：《DK-莱纳·托尔拓 - 1918版》属性/技能/背景/一寸照写入 char.leina；战报《夜战》增加 pl.leina 专属成长说明 secret-panel | public/wiki/entities/entries/char.leina.json, public/wiki/characters/char.leina.png, report.mist-city-record-20260524.json |
| 2026-06-01 | Cursor Agent | master | **CoC 人物卡 UI 优化**: 属性区左侧 1 寸照占位；体格/闪避/伤害加值三列；技能网格卡片布局；成长 +0 常显；详情页悬停提示与可点击 Tooltip | src/features/wiki/CocSheetPanel.tsx, src/utils/coc-sheet.ts, src/types/wiki.ts |
| 2026-06-01 | Cursor Agent | master | **修复 CoC 人物卡折叠无法展开**: 移除 CollapsibleTrigger 上 `preventDefault`（会阻断 Radix 切换）；仅 `stopPropagation` 防冒泡；补充 `data-[state]` 显示样式 | src/features/wiki/CocSheetPanel.tsx, src/components/ui/collapsible.tsx |
| 2026-06-01 | Cursor Agent | master | **CoC 人物卡折叠 UI**: 扩展 `CocSheetData`（attributes/skills/growth/changes）；新增 `CocSheetPanel` 双折叠行与成长 Tooltip；人物列表卡内嵌人物卡；详情页隐藏检索区；`seed:coc-sheets` 写入 6 张 char 示例数据 | src/types/wiki.ts, src/utils/coc-sheet.ts, src/features/wiki/CocSheetPanel.tsx, src/pages/WorldWikiTab/index.tsx, scripts/seed-coc-sheets.ts, public/wiki/entities/entries/char.*.json |
| 2026-06-11 19:49 | Codex | master | **AI NPC 对话 V2**: 主仓 Blog/WorldWiki/AI Chat 统一改为 token session；AI Chat 改为服务端 NPC 下拉、按 NPC+PL 拉取/发送/删除历史；子仓 trpg-ai-gateway 新增 Bearer token 哈希鉴权、NPC 私有上下文、运行时聊天记忆、队列串行、压缩与删除备份；新增服务器日志 skill 与执行计划文档 | src/features/ai/*, src/pages/AiChatTab/index.tsx, src/pages/BlogTab/index.tsx, src/pages/WorldWikiTab/index.tsx, trpg-ai-gateway/*, .claude/skills/check-server-logs.md, docs/superpowers/plans/2026-06-11-ai-npc-chat-v2.md, docs/business.md, docs/tech-decisions.md, user_request.md |
| 2026-06-12 12:00 | Codex | master | **AI 对话头像与人物 Wiki 头像**: 新增稳定文字头像组件；Wiki 人物卡显示 48x48 头像，达米安/塞巴斯补头像、原始人物卡图片与当前车卡数据；AI Chat 消息按 NPC/当前 PL 对应 PC 显示头像，缺头像时使用稳定底色首字头像；子仓 NPC 列表 DTO 增加头像 URL | src/components/StableAvatar.tsx, src/pages/WorldWikiTab/index.tsx, src/pages/AiChatTab/index.tsx, src/features/ai/ai-gateway-client.ts, public/wiki/entities/entries/*.json, public/wiki/characters/*, trpg-ai-gateway/src/*, trpg-ai-gateway/data/npcs/* |
| 2026-06-12 12:00 | Codex | master | **GitHub Actions 部署上传加速**: 完整部署改为先将 `dist` 打包成单个 `tar.gz`，SCP 上传后在服务器 release 目录解压；静态内容部署同样改为打包 `public/blog` 与 `public/wiki` 后单文件上传，减少 SCP 多文件传输开销 | .github/workflows/deploy.yml, .github/workflows/deploy-content.yml, docs/dev-log.md |
