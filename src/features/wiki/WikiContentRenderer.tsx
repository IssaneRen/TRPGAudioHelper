...
function LockedSecretBlock({
  ...
  return (
    <button
      ...
      className="relative my-6 w-full min-w-0 overflow-hidden rounded-xl border border-black/70 bg-black p-4 text-left shadow-inner transition-transform hover:-translate-y-0.5"
    >
...
function LockedInlineSecret({ text }: { text: string }) {
  return (
    <button
      type="button"
      onClick={() => toast("请探索更多故事解锁~", { duration: 1800 })}
      className="inline rounded-sm bg-black px-1.5 py-0.5 align-baseline text-neutral-700"
    >
      {maskSecretText(text)}
    </button>
  );
        ...
        if (token.type === "ref" && token.entryId) {
          ...
          return (
            <Link
              ...
              className="break-words font-medium text-primary underline decoration-primary/40 underline-offset-4 hover:decoration-primary"
            >
              {label}
            </Link>
          );
...
export function WikiContentRenderer({
  ...
  return (
    <div className="wiki-readable space-y-5">
      {blocks.map((block, index) => {
        if (block.type === "heading") {
          return (
            <h3 key={`heading-${index}`} className="break-words font-heading text-xl font-semibold md:text-2xl">
              {block.text}
            </h3>
          );
        }
        ...
        if (block.type === "paragraph") {
          return (
            <p key={`paragraph-${index}`} className="break-words leading-8 text-foreground/95">
              <InlineTokens
                tokens={block.tokens || []}
        ...
        if (block.type === "quote") {
          return (
            <blockquote
              key={`quote-${index}`}
              className="break-words border-l-2 border-primary/40 bg-primary/5 px-4 py-3 italic text-muted-foreground"
            >
              <InlineTokens
                tokens={block.tokens || []}
                currentPlayerId={currentPlayerId}
        ...
        if (block.type === "image") {
          ...
          return (
            <figure key={`image-${index}`} className="my-4 mx-auto w-full max-w-[560px] overflow-hidden">
              <img
                src={src}
                alt={alt}
                loading="lazy"
        ...
        if (block.type === "list") {
          return (
            <ul key={`list-${index}`} className="list-disc space-y-2 pl-5 sm:pl-6">
              {(block.items || []).map((item, itemIndex) => (
                <li key={`item-${itemIndex}`} className="break-words leading-7">
                  <InlineTokens
                    tokens={item}
                    currentPlayerId={currentPlayerId}
                    entriesById={entriesById}
        ...
        if (block.type === "secret-panel") {
          return canRevealSecret(block.playerIds, currentPlayerId, revealAllSecrets) ? (
            <div
              ...
              className="mobile-safe-width rounded-2xl border border-primary/20 bg-primary/5 p-4"
            >
